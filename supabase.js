...
function dbSubscribeRoom(roomCode, onStateChange, onWhisper, onGuestJoin, onHostAction) {
  if (!db) throw new Error('Supabase client not initialised.');
  ...
  if (onGuestJoin) {
    channel.on('broadcast', { event: 'guest_join' }, (payload) => {
      onGuestJoin(payload.payload);
    });
  }
  if (onHostAction) {
    channel.on('broadcast', { event: 'host_action' }, (payload) => {
      onHostAction(payload.payload);
    });
  }
  channel.subscribe();
  return channel;
...
async function dbBroadcastGuestJoin(channel, player) {
  if (!channel) return;
  await channel.send({
    type: 'broadcast',
    event: 'guest_join',
...
async function dbBroadcastHostAction(channel, action, data = {}) {
  if (!channel) return;
  await channel.send({
    type: 'broadcast',
    event: 'host_action',
...
// ═══════════════════════════════════════════════════════════════════════════
// CLOUD QUESTIONS DATABASE (Offline-First Sync)
// ═══════════════════════════════════════════════════════════════════════════

async function dbFetchAllQuestions() {
  if (!db) return null;
  try {
    let all = [];
    let from = 0;
    ...
    console.warn('[Supabase] Could not fetch questions from cloud:', err);
    return null;
  }
}