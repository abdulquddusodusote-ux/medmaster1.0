...
const App = {
  ...
  // ── Cascading Filters (Accordions & Checkboxes) ────────────────────────
  ...
  getUncheckedValues(containerId) {
    ...
    return new Set(Array.from(unchecked).map(cb => cb.value));
  },
  ...
  getFullyFilteredPool(prefix) {
    ...
    const activeSubTopics = this.getCheckedValues(`${prefix}-subtopic-container`);
    ...
    if (!activeTopics.includes('all')) {
      pool = pool.filter(q => activeTopics.includes(q.topic));
    }

    if (!activeSubTopics.includes('all')) {
      pool = pool.filter(q => activeSubTopics.includes(q.subTopic));
    }
  ...
  renderTopicSelection(prefix) {
    ...
    // Remember what the user manually unchecked so it doesn't reset when they add a new course
    ...
    if (Object.keys(topicsByCourse).length === 0) {
      container.innerHTML = '<p class="text-muted mt-1">No topics available.</p>';
    ...
    for (const course of Object.keys(topicsByCourse).sort()) {
      ...
      const checkboxesHtml = topics.map(t => `
        <label class="custom-checkbox">
          <input type="checkbox" class="topic-checkbox" value="${t.replace(/"/g, '&quot;')}" ${!uncheckedTopics.has(t) ? 'checked' : ''}>
          <span>${t}</span>
        </label>
      `).join('');
  ...
  renderSubTopicSelection(prefix) {
    ...
    const uncheckedSubTopics = this.getUncheckedValues(`${prefix}-subtopic-container`);
    const container = document.getElementById(`${prefix}-subtopic-container`);
    if (!container) return;

    if (Object.keys(subTopicsByTopic).length === 0) {
       container.innerHTML = '<p class="text-muted mt-1">No sub-topics available.</p>';
    ...
    for (const topic of Object.keys(subTopicsByTopic).sort()) {
      const subTopics = Array.from(subTopicsByTopic[topic]).sort();
      const checkboxesHtml = subTopics.map(st => `
        <label class="custom-checkbox">
          <input type="checkbox" class="subtopic-checkbox" value="${st.replace(/"/g, '&quot;')}" ${!uncheckedSubTopics.has(st) ? 'checked' : ''}>
          <span>${st}</span>
        </label>
      `).join('');
  ...
  bindFilterEvents(prefix) {
    ...
    // Handle Clicks (Pills & Accordion Toggles)
    ...
    // Handle Changes (Checkboxes & Selects)
